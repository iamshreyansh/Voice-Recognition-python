...
def WAVin(B):   # Read the audio from the WAV file and store the data into the array and read data from the array
    global AUDIOdevin
    global AUDIOdevout
    global AUDIOsignal1
    global AUDIOstatus
...
def STOREcsvfile():     # Store the trace as CSV file [frequency,dB value] 
    ...
    Wfile = open(filename,'a')                        # Open output file
...
def ASKWAVfilename():
    global use
    filename = askopenfile(filetypes=[("WAVfile","*.wav"),("allfiles","*")])
...
if WAVinput == 0:
    ...
    WAVin(0)
    subprocess.call("speccl2.py", shell=True)
